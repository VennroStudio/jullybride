...
$product_story_media_url = static function (mixed $media, string $size = 'full'): string {
    if (is_array($media)) {
        if (!empty($media['id'])) {
            $url = $size === 'full'
            ...
            <ul class="owl-carusel-added-story owl-carousel owl-theme" id="owl-carusel-added-story_<?php echo esc_attr($story_index); ?>">
                <?php if (have_rows('karusel')) : ?>
                    ...
                    while (have_rows('karusel')) :
                        ...
                        $photo_url = $product_story_media_url($photo, 'carousel-story');
                        $photo_alt = is_array($photo) ? (string) ($photo['alt'] ?? '') : '';
                        $video_url = $product_story_media_url($video);
                        ?>
                        <li>
                            <?php if ($video_url) : ?>
